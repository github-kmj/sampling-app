...
def format_phone(raw):
    ...
    if digits.startswith("02"):
        local = digits[2:]
        if len(local) == 7:
            return f"02-{local[:3]}-{local[3:]}"
        elif len(local) == 8:
            return f"02-{local[:4]}-{local[4:]}"
        else:
            return f"02-{local}"
    # 3자리 지역번호
    elif len(digits) == 10:
...
def build_address(row):
    def s(v):
        """값 정제: NaN/None/빈값 → 빈 문자열, 숫자는 정수형으로"""
        if pd.isna(v):
            return ""
        v = str(v).strip()
        if v in ("nan", "None", "NaN", ""):
            return ""
        # 정수처럼 보이는 소수점 제거 (예: 2.0 → 2)
        try:
            f = float(v)
            if f == int(f):
                return str(int(f))
    ...
    haengjeong  = s(row.get("행정구역", ""))   # 예: "경기도 남양주시 진접읍"
    road        = s(row.get("도로명", ""))
    ...
    # 행정구역에서 마지막 단어(읍면동) 제거 → 시도+시군구만
    def drop_last_word(text):
        parts = text.split()
        if len(parts) >= 3:
            return " ".join(parts[:-1])  # 마지막 읍면동 제거
        return text

    if road and main_no:
        # 도로명 주소: 행정구역(읍면동 제거) + 도로명 + 번지
        ...
        if not prefix or not road or not main_no:
            inaccurate = True
...
with tab1:
    ...
    if raw_file:
        try:
            # 자동 헤더 행 탐지 (첫 번째 비어있지 않은 행)
            df_peek = pd.read_excel(raw_file, header=None, nrows=10)
            header_row = 0
            for i, row in df_peek.iterrows():
                if row.notna().sum() > 3:
                    ...
                    break
            raw_file.seek(0)
            df_raw = pd.read_excel(raw_file, header=header_row)
            df_raw.columns = df_raw.columns.astype(str).str.strip()
            st.success(f"✅ 파일 로드 완료 — {len(df_raw):,}개 행, {len(df_raw.columns)}개 열 (헤더: {header_row+1}행)")
    ...
    if df_raw is not None and st.button("🔧 명부정리 시작", type="primary"):
        ...
        if biz_col:
            ...
            for (biz, field), group in dup_df.groupby([biz_col, "_업종분야"]):
                if industry_col:
                    # 같은 분야 내에서 가장 많은 산업대분류코드 기준으로 후보 선정
        ...
        if existing_addr:
            last_addr_col = existing_addr[-1]
            insert_pos    = df.columns.get_loc(last_addr_col) + 1
...
with tab2:
    ...
    st.markdown('<div class="info-box">• <b>전국사업체조사</b>: 1단계 명부정리 후 다운받은 <b>깨끗한 명부</b>를 업로드하세요.<br>• <b>공시대상</b>: 별도 정리 없이 <b>원본 파일을 바로</b> 업로드하세요.</div>', unsafe_allow_html=True)

    col1, col2 = st.columns(2)
    with col1:
        file_nat = st.file_uploader("전국사업체조사 깨끗한 명부", type=["xlsx"], key="s_nat")
    with col2:
        file_pub = st.file_uploader("공시대상 깨끗한 명부", type=["xlsx"], key="s_pub")

    st.subheader("층별 할당표본 파일 업로드")
    st.markdown("""
    <div class="info-box">
    중분류 × 매출액규모별 할당표본 수 파일을 업로드하세요.<br>
    형식: <b>중분류 | 구분 | 6 | 7 | 8 | 9</b> (구분: <b>전국</b> 또는 <b>공시</b>)
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("🎯 표본추출 시작", type="primary", disabled=not ready2):
        ...
        try:
            ...
            df_alloc = smart_read(file_alloc)
            st.success(f"✅ 전국: {len(df_nat):,}개 / 공시: {len(df_pub):,}개")
        except Exception as e:
            st.error(f"파일 로드 오류: {e}"); st.stop()

        # ── 할당표본 파싱 ──
        try:
            alloc_cols = list(df_alloc.columns)
            mid_col, div_col, size_cols = alloc_cols[0], alloc_cols[1], alloc_cols[2:]
            alloc_long = df_alloc.melt(id_vars=[mid_col, div_col], value_vars=size_cols,
                                       var_name="매출액규모", value_name="할당표본")
            alloc_long.columns = ["산업중분류코드", "구분", "매출액규모", "할당표본"]
            for c in ["산업중분류코드", "매출액규모"]:
                alloc_long[c] = alloc_long[c].astype(str).str.strip()
            alloc_long["할당표본"] = pd.to_numeric(alloc_long["할당표본"], errors="coerce").fillna(0).astype(int)
            alloc_long = alloc_long[alloc_long["할당표본"] > 0]
        except Exception as e:
            st.error(f"할당표본 파싱 오류: {e}"); st.stop()

        # ── 전처리 ──
        for df_ in [df_nat, df_pub]:
            df_["산업중분류코드"] = df_["산업중분류코드"].astype(str).str.strip()
            df_["매출액규모"]     = df_["매출액규모"].astype(str).str.strip()
            df_["산업분류코드"]   = df_["산업분류코드"].astype(str).str.strip() if "산업분류코드" in df_.columns else ""
            df_["종사자규모"]     = pd.to_numeric(df_["종사자규모"], errors="coerce").fillna(0)
            df_["도로명주소"]     = df_.get("통합주소", df_.get("도로명주소", pd.Series([""] * len(df_)))).astype(str)
            df_["사업체구분코드"] = df_["사업체구분코드"].astype(str).str.strip()
        ...
        # ── 사업체구분코드 1,2 우선 추출 함수 ──
        def extract_layer(layer_df, n, dup_pids):
            """
            코드 1,2 우선 추출 → 부족하면 코드 3에서 추가
            """
        ...
        alloc_nat  = alloc_long[alloc_long["구분"].str.contains("전국", na=False)]
        nat_samples, nat_log = [], []

        for _, row in alloc_nat.iterrows():
            mid, size, n = str(row["산업중분류코드"]), str(row["매출액규모"]), int(row["할당표본"])
            layer = df_nat[(df_nat["산업중분류코드"] == mid) & (df_nat["매출액규모"] == size)].copy()
            layer = layer.sort_values(["산업분류코드", "종사자규모", "도로명주소"])
            N     = len(layer)
            nat_log.append({"중분류": mid, "규모층": size, "모집단": N, "할당표본": n})
            if N == 0: continue
        ...
        # ── 공시 추출 ──
        st.info("공시대상 표본추출 중...")
        alloc_pub  = alloc_long[alloc_long["구분"].str.contains("공시", na=False)]
        pub_samples, pub_log = [], []

        for _, row in alloc_pub.iterrows():
            mid, size, n = str(row["산업중분류코드"]), str(row["매출액규모"]), int(row["할당표본"])
            layer = df_pub[(df_pub["산업중분류코드"] == mid) & (df_pub["매출액규모"] == size)].copy()
            layer = layer.sort_values(["산업분류코드", "종사자규모", "도로명주소"])
            N     = len(layer)
            pub_log.append({"중분류": mid, "규모층": size, "모집단": N, "할당표본": n})
            if N == 0: continue
